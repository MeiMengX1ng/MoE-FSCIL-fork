...
给定按阶段（session）`S = {0, 1, …, T}` 顺序到达的数据流，基类阶段 `t = 0` 提供充足的标注数据，增量阶段 `t ≥ 1` 仅提供少量新类样本（典型 N-way K-shot，`K = 1`）。模型在每个阶段 `t` 需同时识别已学类别 `C^(0:t)` 中所有类，且无法回放旧类原始数据。
...
| 阶段路由模块 | [router.py:9-169](models/vmoe/router.py#L9-L169) |
| 阶段解耦分类模块 | [classifier.py:6-57](models/vmoe/classifier.py#L6-L57) |
| 基于记忆存储器的阶段判别器 (MSD) | [router.py:67-94](models/vmoe/router.py#L67-L94) |
| 基于分布判别的阶段判别器 (DSD) | [router.py:97-133](models/vmoe/router.py#L97-L133) |
| 原型近邻插值特征增广 | [augment.py:5-45](models/vmoe/augment.py#L5-L45) |
...
所有图像统一缩放至 224×224，增广策略为随机裁剪 + 随机水平翻转 + 随机擦除。
...
| 优化器（路由） | SGD | Adam (lr=3×10⁻³, wd=1×10⁻⁴) |
...
| `-lora_alpha` | 16 | 专家输出缩放系数 α（实际缩放为 α/r） |
...
本方法包含多个相互独立的可消融模块，建议在论文中按以下方式报告消融结果：
